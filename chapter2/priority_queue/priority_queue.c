...
void pq_enqueue(PriorityQueue *pq, PQNode *node)
{
    ...
    memcpy(pq->node[current_pos].data, node->data, sizeof(node->data));
...
{
    for (int i = 0; i < pq->used_size; i++) {
        printf("%d %s ", pq->node[i].priority, pq->node[i].data);
    }
}

int main(void)
{
    PriorityQueue *pq = create_queue(3);
    PQNode pop = {0};
    PQNode nodes[7] = 
    {
        {34, "aaaa"},
